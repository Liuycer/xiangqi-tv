...
难度上限与单次思考预算：简单为深度 2 / 350ms，普通为深度 3 / 1100ms，困难为深度 5 / 3200ms，大师为深度 7 / 7000ms。这里的深度是搜索上限；界面会显示真机实际完成深度。时间耗尽时使用最后完整搜索深度的结果；即使第一层未完成也会返回预先排序的合法应手。人机模式悔棋会撤回 AI 与玩家各一手，让玩家重新选择。完整规则仍由独立 `RuleEngine` 负责。

操作区提供悔棋、重新开始、对战模式、AI 难度四项操作，AI 难度可以在简单、普通、困难和大师之间循环。切换本地双人/人机模式会开始一盘新棋。Android 壳已声明鼠标/非触摸设备可用，并同时提供普通 Launcher 与 Leanback Launcher 入口。
