...
当前进度：Phase 25–41 已完成开发。游戏以真人棋手 Profile 为核心运行排位自适应模式；Phase 29–35 完成可靠性、历史与档案恢复，Phase 36 按中国象棋协会 2020 规则增加攻击性循环裁定，Phase 37 修复历史分页，Phase 38–40 增加未完成对局续玩及生命周期并发保护，Phase 41 增加胜局烟花与终局音效。
...
- 历史对局：服务器保存对局时间、结果、评级、棋谱和 AI 分析；客户端每页按需加载 20 局，支持鼠标/遥控器继续分页、逐手回放，并可恢复有效的未完成对局。
- 胜局庆祝：人机对战中玩家红方获胜、本地双人任一方获胜时播放 2.5 秒轻量烟花与合成音效；AI 获胜使用低调结束音，和棋不放烟花。
...
生产备份与 CORS 加固见 [Phase 29 可靠性修复报告](docs/phase-29-reliability.md)。

离线同步队列修复见 [Phase 30 outbox 可靠性报告](docs/phase-30-outbox.md)。

归属校验、重复局面及运行时恢复见 [Phase 31 加固报告](docs/phase-31-hardening.md)。

空对局过滤与清理见 [Phase 32 历史对局修复报告](docs/phase-32-empty-games.md)。

设置面板精简见 [Phase 33 设置界面报告](docs/phase-33-settings-ui.md)。
...
未完成对局续玩见 [Phase 38–40 未完成对局续玩报告](docs/phase-38-40-resumable-games.md)。
...
Phase 38–40 已完成未完成对局续玩。服务端 API 0.8.0 / SQLite schema v7 使用 revision 防止旧 outbox 或并发客户端覆盖已恢复棋局，续局前会重新校验归属、完整棋谱和终局状态；客户端在历史详情中提供鼠标与遥控器“继续这盘对局”，恢复棋盘、棋谱、当前回合以及该局冻结的自适应 AI 参数。前端 100 项、服务端 63 项测试及 Android Lint 全部通过；生产 API 续局、幂等重试和 revision 409 冲突闭环验收通过，最终 APK 已完成鼠标与遥控器真机续局验收。

Phase 35 已增加本机同步失败管理和 Profile 恢复机制。永久 4xx 隔离记录会按对局归组显示在“历史对局”的“同步异常”入口，可逐局删除或清空全部诊断，不影响正常历史与后续同步；“设置与棋谱”仍只保留音效和动画。棋手档案可生成一次性 80-bit 恢复码，服务器仅保存摘要；在新设备输入后会整体转移段位、评级、历史和分析，并原子销毁旧码。新码会使旧码失效，删除档案也会清除恢复凭据，目标设备仍受 6 档案上限保护。前端 93 项、服务端 60 项测试及 1080P/720P 布局检查通过。

Phase 34 已为 API 增加独立中国象棋规则验证。云端不再信任客户端上传的 UCI 棋谱、当前行棋方或终局结果，而是从初始 FEN 逐手重放，校验七类棋子的移动、阻挡、九宫与过河限制、蹩马腿、塞象眼、自陷将军、将帅照面、重复局面和长将。快照的 `currentPlayer` 必须与重放一致；completed 记录必须确实形成将死、困毙、长将判负或三次重复和棋，且结果与终止原因一致。AI 走棋和分析请求也会在进入 Pikafish 前完成同样的棋谱校验。服务端 56 项测试全部通过。

Phase 33 已精简“设置与棋谱”面板。设置区只保留落子音效和走子动画两个功能开关；“返回对局”按钮移除右侧“关闭”文字；重置水平只在棋手档案详情中提供。遥控器焦点同步缩减为音效、动画、返回三项，删除的控件无法再通过 OK 触发。前端 91 项测试和生产构建通过；使用当前 Token 的 APK 已覆盖安装，并完成电视端实际画面验收。

Phase 32 已修复历史中的零手数空对局。客户端只在第一手棋产生后才把开局写入同步队列；未走子就重新开始、切换模式/棋手或退出时不会产生网络记录。服务端收到旧客户端的零手数放弃请求会直接删除该局，并从历史分页中过滤遗留空记录；至少走过一手的中途退出仍正常保留。前端 91 项、服务端 37 项测试全部通过。生产数据库已先备份再删除 23 条空记录，剩余 25 条有效对局；API 0.4.1 的临时空局验收为 start=200、finish=204、残留=0。使用重置后 Token 的最终 APK 已覆盖安装并正常启动。

Phase 31 已完成四项可靠性加固。所有 Profile 所属接口强制携带并核验 `deviceId + playerId`；重复局面裁定改为检查当前局面的最近三次出现，不再错误要求循环路径等长，并能判定跨不同长度路径形成的单方连续长将；赛后分析 worker 会在领取任务或保存失败状态发生数据库异常后继续运行；遥控器 OK 与鼠标共用禁用态守卫，不能在分析进行中重复提交或触发已禁用的重置。前端 88 项、服务端 33 项测试全部通过。最终 Debug APK 已覆盖安装到盒子，生产 API 已部署并通过缺失设备、错误归属与正确归属三类验收。

Phase 30 已完成客户端 outbox 可靠性修复。400、404、409、410、413、415、422 等不可恢复响应会将对应对局的待传操作移入本地隔离区，并继续同步下一局；依赖于同一失败请求的 snapshot/finish 会以 424 一并记录，避免连续产生确定性错误。401/403、408、425、429、5xx、超时和网络错误仍保留在原队列等待后续重试，正常 FIFO 顺序和 4.2 秒节流保持不变。前端 86 项测试全部通过；真机实测 422 坏请求隔离后，下一条合法请求按节流间隔返回 200。最终 Debug APK 已覆盖安装到盒子。

Phase 29 已完成生产可靠性修复。SQLite 备份脚本以只读连接调用在线备份 API，压缩后会临时恢复并执行 `PRAGMA quick_check`，只有校验成功的文件才正式发布；systemd 服务允许 WAL 所需的 `-shm` 辅助文件写入。FastAPI 已允许 PATCH、DELETE 和 OPTIONS，Nginx 的 OPTIONS 不计入限流，429 响应会返回带 CORS 与安全头的 JSON。生产环境已实际生成 74KB 备份，在线库与备份均为 48 局且完整性检查通过。前端仍为 76 项测试，服务端增至 28 项测试并全部通过。

Phase 25–28 已完成 Profile 排位模式。SQLite schema v5 为棋手增加设备归属、名称、头像、软删除和最近使用时间，并能把旧玩家数据无损迁移为默认棋手。客户端冷启动先选择实际玩家，所有新请求使用 `deviceId + profileId` 核验数据归属；评级、历史、棋谱和 AI 复盘均按 Profile 隔离。原固定难度和自定义深度入口已移除，人机对局只使用 A0–A7 排位自适应。右侧对局信息已针对远距离观看放大，七项功能菜单完整保留在 1080P 电视安全区内。历史列表已修复条目裁切，服务器 UTC 时间会稳定转换为北京时间，逐手评价使用中文标签；离线 outbox 按 VPS 限流节奏在后台补传，历史读取不再因积压队列出现 `Failed to fetch`。Android 构建会拒绝打包缺少云端配置的 Web 资源。最终 APK 已完成盒子鼠标与遥控器实机验证。
...
Phase 41 已完成胜局庆祝。人机模式仅玩家红方获胜放烟花，AI 黑方获胜只播放低调结束音；本地双人任一方获胜都放烟花，和棋不放。Canvas 采用 1280×720 内部上限、24fps、2.5 秒和最多 96 粒子，切换页面、重新开始、进入后台或关闭动画会立即清理。真机播放期间 Total PSS 约 120MB，播放结束约 108MB；正式 APK 已覆盖安装。详见 [Phase 41 胜局烟花与终局音效](docs/phase-41-victory-fireworks.md)。

Phase 12 已完成体验优化。应用使用 Web Audio API 实时合成选棋、落子、吃子、将军、胜利和失败六类短音，不携带音频资源。棋子采用 150ms 轻量走子动画；音效和动画都能在“设置与棋谱”面板独立关闭，并保存在本机 WebView 存储中。
