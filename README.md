...
Phase 11 已完成性能优化。AI 叶子节点不再重复生成合法走法，截止时间改为逐节点检查，并复用根走法；真机简单、普通、困难分别为约 366ms、1104ms、2805ms。困难档从 Phase 10 的 D2 / 512 节点提升至 D3 / 2316 节点，同时耗时从 4329ms 降至 2805ms。
...
难度上限与单次思考预算：简单为深度 2 / 350ms，普通为深度 3 / 1100ms，困难为深度 4 / 2800ms。时间耗尽时使用最后完整搜索深度的结果；即使第一层未完成也会返回预先排序的合法应手。人机模式悔棋会撤回 AI 与玩家各一手，让玩家重新选择。完整规则仍由独立 `RuleEngine` 负责。

操作区提供悔棋、重新开始、对战模式、AI 难度四项操作。切换本地双人/人机模式会开始一盘新棋。Android 壳已声明鼠标/非触摸设备可用，并同时提供普通 Launcher 与 Leanback Launcher 入口。
